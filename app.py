...
if st.button("Generate ARIMA Forecast"):
    ...
    # Always forecast AFTER last historical point
    ...
    st.download_button("Download CSV", csv, "forecast_30_days.csv", "text/csv")

    # Plot correctly
    fig, ax = plt.subplots(figsize=(12,5))
    ax.plot(df["Date"], df["Close"], label="Historical")
    ax.plot(forecast_df["Date"], forecast_df["Forecast"], marker="o", color="orange", label="Forecast")
    ax.legend()
    ax.grid()
    st.pyplot(fig)
